...
export function useTripPage() {
  ...
  const chatMessages = ref([
    "Hi Jordan. I can help shape your next trip. Tell me a destination, dates, and who’s coming.",
  ]);
  ...
  const receiptItems = ref<ReceiptItem[]>([
    { name: "Receipt item", price: 12, member: "JM", translated: false, translation: "" },
  ]);
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        const worker = await createWorker("eng", 1, {
          logger: (message) => {
            receiptProgress.value = Math.round(message.progress * 100);
          },
        });
        ...
        if (parsed.length) receiptItems.value = parsed;
  ...
  function receiptTotal(member?: string) {
    const subtotal = receiptItems.value
  ...
  function confirmReceipt() {
    ...
    const base = receiptItems.value.reduce((sum, item) => sum + item.price, 0);
    selectedEvent.value.bill = receiptItems.value.map((item) => ({
      name: item.name,
      price: item.price + (base ? (item.price / base) * receiptTax.value : 0),
      member: item.member,
      settled: false,
    }));
  ...
  function addBillItem() {
    ...
    selectedEvent.value.bill.push({
      name: billItemName.value.trim(),
      price: Number(billItemPrice.value),
      member: billItemMember.value,
      settled: false,
    });
    billItemName.value = "";
    billItemPrice.value = "";
  ...
  function translateReceiptItem(item: ReceiptItem) {
    item.translation = item.name === "Receipt item" ? "Shared meal" : item.name;
    item.translated = true;
  }
  async function showMapPanel() {
    showMap.value = true;
    leaflet ??= await import("leaflet");
    await nextTick();
    const firstEvent = events.value[0];
    if (!mapElement.value || !leaflet || !firstEvent) return;
    map = leaflet.map(mapElement.value).setView(firstEvent.coords, 11);
    leaflet
      .tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: "© OpenStreetMap contributors",
      })
      .addTo(map);
    mapLayers = leaflet.layerGroup().addTo(map);
    const points = events.value.map((event) => event.coords);
    events.value.forEach((event, index) =>
      leaflet!
        .marker(event.coords)
        .bindPopup(`<strong>${index + 1}. ${event.title}</strong><br>${event.place}`)
        .addTo(mapLayers!),
    );
    leaflet.polyline(points, { color: "#d5cfc4", weight: 3, dashArray: "5 8" }).addTo(mapLayers);
  ...
  async function loadRealRoute(points: [number, number][]) {
    if (!mapLayers || !leaflet || points.length < 2) return;
    routeStatus.value = "Finding best walking route…";
    try {
      const coordinates = points.map(([lat, lon]) => `${lon},${lat}`).join(";");
      const response = await fetch(
        `https://router.project-osrm.org/route/v1/foot/${coordinates}?overview=full&geometries=geojson&alternatives=true`,
      );
      ...
      const best = routes.reduce((winner, route) =>
        route.duration < winner.duration ? route : winner,
      );
      leaflet
        .polyline(
          best.geometry.coordinates.map(([lon, lat]) => [lat, lon] as [number, number]),
          { color: "#d9644e", weight: 5, opacity: 0.9 },
        )
        .addTo(mapLayers);
  ...
  function sendChat() {
    ...
    chatMessages.value.push(
      prompt,
      /trip|travel|visit|food|place|day|route|hotel|flight/i.test(prompt)
        ? "I can shape that into your trip. Confirm a destination, dates, and pace, then I’ll suggest activities."
        : "I only help with this trip.",
    );
    note.value = "";
  }
  function addSuggestion(suggestion: (typeof nearbySuggestions)[number]) {
    addEvent(
      suggestion.title,
      `Nearby · ${suggestion.distance} · ${suggestion.price}`,
      suggestion.tag,
    );
  }
  function copyInvite() {
